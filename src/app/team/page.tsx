...
// 팀이 아직 없는 사용자가 처음 만나는 화면. 팀 생성은 관리자(PLATFORM_ADMIN_EMAIL)만 할 수 있고
// (teams_insert_self_owner RLS로도 강제), 그 외 사용자는 초대 링크로만 팀에 합류할 수 있다 —
// 검색 후 가입신청을 보내는 흐름은 폐지했다(team_members_insert_self 정책도 함께 제거).
// 단일 팀 사용을 전제로 하므로, 이미 승인된 팀이 있으면 바로 대시보드("/")로 돌려보낸다
// (여러 팀 동시 소속 자체는 DB/team-switcher에 여전히 남아있지만, 이 페이지에서 새로 팀을
// 추가하는 흐름은 더 이상 제공하지 않는다) — 단, 관리자만 테스트용 팀을 여러 개 만들어야 해서
// 이 리다이렉트에서 예외로 둔다.
      ...
      <div className="flex w-full max-w-sm flex-col gap-3">
        ...
        {isPlatformAdmin ? (
          ...
          <p className="text-center text-xs text-zinc-500">
            팀 생성은 관리자만 할 수 있어요. 팀장에게 초대 링크를 받아 가입해주세요.
          </p>
        )}
        {isPlatformAdmin && (
          <p className="text-center text-xs text-zinc-500">
